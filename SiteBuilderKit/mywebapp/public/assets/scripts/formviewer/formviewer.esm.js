...
class FormViewer {
  constructor(element, settings = {}) {
    const defaults = {
      ...
      // onChange: (json)=>{},
      formHeader: true,
      fileUploadText: 'Drag and drop your file here, or click to select.',
      model: 'gpt-4o-mini',
      language: 'English',
      ...
      // scrapeCommandUrl: '',
      scrapeLimit: 3,
      scrapeExclude: ['youtube.com', 'reddit.com'] // Domains to exclude
    };
  ...
  view(jsonText) {
    ...
    this.inputListener = () => {
      ...
      this.debounceTimeout = setTimeout(() => {
        const json = this.get();
        if (this.settings.onChange) this.settings.onChange(json);
      }, 300);
    };
    this.changeListener = () => {
      const json = this.get();
    ...
    if (preview) {
      ...
      if (!hideHeader) {
        ...
        if (json.description) {
          ...
          tagline.innerText = json.description || '';
      ...
      //     form.method = 'POST';
      //     form.action = this.settings.formAction || '';
      // } else {
      //     form = document.createElement('div');
      // }

      formWrapper.className = 'form-wrapper';
      formWrapper.style.flexFlow = 'wrap';
      preview.appendChild(formWrapper);
      let idx = 1;
      if (json.elements) json.elements.forEach(field => {
        ...
        formWrapper.appendChild(divField);
        const type = field.type;
        let fieldName = field.name;

        // prevent same name 'question'
        let elm = formWrapper.querySelector(`[name="${fieldName}"]`);
        if (elm) {
          idx++;
        ...
        if (type === 'select') {
          ...
          if (field.choices) {
            field.choices.forEach((choice, index) => {
              ...
              const checkIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" class="size-5 flex-none"><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M12 7a5 5 0 1 1 -4.995 5.217l-.005 -.217l.005 -.217a5 5 0 0 1 4.995 -4.783z"></path></svg>';
        ...
        if (type === 'multi-select') {
          ...
          if (field.choices) {
            field.choices.forEach((choice, index) => {
              ...
              div.appendChild(peerSpan);
              const checkIcon = `<svg  xmlns="http://www.w3.org/2000/svg"  width="24"  height="24"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="4"  stroke-linecap="round"  stroke-linejoin="round"  class="size-5">
        ...
        if (type === 'switch') {
          ...
          switchElm.className = 'switch outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary-width focus:ring-primary-color transition duration-150 ease-in-out';
        ...
        if (type === 'dropdown') {
          ...
          option.value = '';
          option.innerText = field.placeholder;
        ...
        if (type === 'file') {
          ...
          fileInputWrapper.addEventListener('dragover', event => {
            ...
            // fileInputWrapper.classList.add('bg-gray-100');
          });

          // fileInputWrapper.addEventListener('dragleave', () => {
          //     fileInputWrapper.classList.remove('bg-gray-100');
          // });
          ...
          const selectedCategories = field.allowedFileTypes || ['images', 'videos', 'documents', 'others'];
          ...
          // function onUpload(file) { // Custom file upload handler
      ...
      if (this.settings.captchaContainer) {
        ...
        formWrapper.appendChild(divCaptcha);
      }
      //----
      ...
      if (json.useSubmitButton) {
        ...
        if (this.steps && this.steps.length > 0) {
          ...
          if (form) {
            form.addEventListener('submit', async e => {
              e.preventDefault();
              this.process(btnSubmit, buttonText);
            });
          } else {
        ...
        //         this.process(btnSubmit, buttonText);
        //     } else {
        //         if(this.settings.onSubmit) this.settings.onSubmit(e);
        //     }
        // });
      }
    }
  ...
  async search(query) {
    const apiUrl = this.settings.searchCommandUrl; // Replace with your server's URL if deployed
    ...
    const organicResults = (result && result.organic_results ? result.organic_results.map(item => ({
      title: item.title,
      link: item.link,
      snippet: item.snippet
    })) : []) || [];

    // if(this.settings.consoleLog) console.log(organicResults);
  ...
  async scrapeUrls(urls) {
    ...
    const response = await fetch(this.settings.scrapeCommandUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
`;
    let functs = [{
      ...
      parameters: {
        ...
        properties: {
          ...
          url: {
            type: 'string',
            description: 'If scrape needed, return the url.'
          },
          url2: {
            type: 'string',
            description: 'If scrape needed, return the url.'
  ...
  async getRelevantInfo(prompt) {
    if (!this.settings.searchCommandUrl && !this.settings.scrapeCommandUrl) return '';
    let context = '';
    let args = await this.checkPrompt(prompt);
    if (args.needsearch && this.settings.searchCommandUrl) {
      ...
      // Filter URLs, excluding any that match the domains in the exclude array
      const filteredUrls = results.map(item => item.link) // Extract array of URLs
      .filter(url => !exclude.some(domain => url.includes(domain))); // Exclude specified domains
      ...
      //     console.log('Context from Search & Scrape the search results:');
      //     console.log(result);
      // }
    } else if (args.needscrape && this.settings.scrapeCommandUrl) {
      let urls = [];
      ...
      // if(this.settings.consoleLog) {
      //     console.log('Context from Scrape:');
      //     console.log(result);
      // }
    }
    return context;
  }
  async process(btnSubmit, buttonText) {
    const resultElement = this.settings.resultElement;
    ...
    requests.forEach(item => {
      ...
      formValues.forEach(field => {
        output = output.replaceAll(`{{${field.name}}}`, field.value); // also replace normal {{tags}}
      });
      item.prompt = output;
    });

    /*
    const typingQueue = []; // Queue for incoming chunks
    let isTyping = false;   // Flag to check if currently typing
    
    // Function to type each chunk with a delay
    const typeText = async (text, container, delay = 50) => {
        container.innerHTML += text; // Append the full chunk
        await new Promise(resolve => setTimeout(resolve, delay)); // Delay before next chunk
    };
    */

    /*
    // Process the queue and type the chunks sequentially
    const processQueue = async (container) => {
        if (isTyping || typingQueue.length === 0) return; // Do nothing if already typing or queue is empty
        isTyping = true;
         let currentText=''; //outout
        while (typingQueue.length > 0) {
            const nextChunk = typingQueue.shift(); // Get the next chunk
            currentText+=nextChunk;
             // await typeText(nextChunk, container); // Display the chunk in the right container
            const htmlContent = marked.parse(currentText);
            container.innerHTML = htmlContent;
            await new Promise(resolve => setTimeout(resolve, 50)); // Delay before next chunk
        } 
         isTyping = false;
        // console.log(currentText);
        return currentText;
    };
     const handleRequest = async (prompt, context, system, functs) => {
         // Create a new div to contain the result of this request
        const resultDiv = document.createElement('div');
        resultDiv.classList.add('result-container'); // Optionally style it
        resultElement.appendChild(resultDiv);
    
        // Reset the typing queue for this request
        typingQueue.length = 0; // Clear previous chunks
        typingQueue.currentContainer = resultDiv; // Store the reference to the current result container
        ...
        await this.request(
            ...
            functs,
            (chunk) => {
                // Add the chunk to the queue for typing
                typingQueue.push(chunk);
                // processQueue(resultDiv); // Start processing the queue for this specific result
            }
        );
                
        // Start processing the queue (typing) once the request has completed
        let output = await processQueue(resultDiv); // Start processing the queue for this specific result
        return output;
    };
    */
    ...
    const handleRequest = async (prompt, context, system, functs) => {
      ...
      let currentText = '';
      let addContext = await this.getRelevantInfo(prompt);
...
`;
      ...
      // Request processing
      await this.request(prompt, context, system, functs, async chunk => {
    ...
    const handleRequestQueue = async requests => {
      let previousResults = []; // Store results of each step
      ...
      for (let i = 0; i < requests.length; i++) {
        ...
        let result = await handleRequest(prompt, context, system, functs);
  ...
  loadWorkflow(jsonText) {
    ...
    const sendCommandUrl = json.sendCommandUrl;
    const sendCommandStreamUrl = json.sendCommandStreamUrl;
    const searchCommandUrl = json.searchCommandUrl;
    const scrapeCommandUrl = json.scrapeCommandUrl;
    this.settings.sendCommandUrl = sendCommandUrl;
    this.settings.sendCommandStreamUrl = sendCommandStreamUrl;
    this.settings.searchCommandUrl = searchCommandUrl;
    this.settings.scrapeCommandUrl = scrapeCommandUrl;
    this.steps = steps;
  }
  getFormValues() {
  ...
  get_bak() {
    // depend on form element
    const form = this.element.closest('form');
    if (!form) {
      alert('Wrap the form inside a <form> element.');
      return {};
    }
    const formData = new FormData(this.element.closest('form')); // Collect form data
  ...
  async send(question, context, system, functs) {
  ...
  async request(question, context, system, functs, onData) {
    this.controller = new AbortController();
    this.signal = this.controller.signal;
    let temperature = 0.6;
    let topP = 0.9;
